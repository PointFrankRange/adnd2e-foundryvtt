...
- **Rename the two DM-run actor types to "Character NPC" and "Monster NPC".** From a DM's perspective every figure not controlled by a player is an NPC, monsters included; the split here is only how the figure is built. Today's `npc` type (class levels, ability scores, derived THAC0/saves, real equipment) should read **Character NPC**, and today's `creature` type (Monstrous Manual stat block: Hit Dice, flat THAC0/AC/saves, attack list) should read **Monster NPC**. Labels only (actor-type names, sheet titles, create dialog) — no data-model change or migration. Decided 2026-09-26; for a future pass.
