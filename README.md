...
| **6. NPC / monster sheet + bestiary scaffolding** | ✅ Complete | Single-page **Monster NPC** stat-block sheet (the `creature` type, fully editable), streamlined 3-tab **Character NPC** sheet (the `npc` type), empty Bestiary compendium folder + authoring docs |
...
- **Monster NPC inventory (designed, next pass).** A Monster NPC (the `creature` type) has no race, class or proficiencies, but should carry an inventory and equip weapons and armor. Decided 2026-09-26: each **equipped weapon becomes an extra attack row** beside the stat-block attacks, rolled with the monster's THAC0 and the weapon's damage (so a disarm genuinely unequips it); **equipped armor does not change AC** — the Monstrous Manual stat-block AC already includes typical gear, and the GM edits it by hand when needed.
- **A relayed disarm against a Monster NPC (creature) logs "applied a disarm" but changes nothing** — creatures use stat-block attacks, not weapon items, so there is nothing to unequip. Harmless; the log could say so or the disarm could be skipped for creature targets.
