...
- Produces:
  ...
  - `ClassChassis` gains: `maxLevel: number | null` (class-intrinsic level cap; `null` = uncapped), `spellStartLevel: number | null` (first level a caster gets a spell; `null` **only** for non-casters — a from-level-1 caster sets `1`), `spellProgressionId: SpellProgressionId | null` (the reliable non-caster signal), `thiefSkillAccess: readonly ThiefSkill[] | null` (`null` = **no thieving skills**; the Thief carries an explicit eight-member list so `chassis.thiefSkillAccess ?? []` is never wrong).
