...
- `thief-skills.ts` — the table `const`s have no executable statements (the shared `skillRow` helper is exercised by every Table-27 and Table-29 entry); `?? 0` both sides (empty DEX-13 row vs a populated row). The `Math.min`/`Math.max` DEX clamp emits no v8 branch counters, so 100%-branches cannot vouch for it — tests exercise it explicitly at both ends anyway (DEX 25 → clamped to 19; DEX 3 → clamped up to 9, reachable because `assertAbilityScore` permits 1-25 and knows nothing of the thief's DEX-9 class minimum). `resolveThiefSkill`'s `Math.min` at and below 95; `backstabMultiplier`'s four bands + the `assertLevel` throw; `pickPocketsDetectionThreshold`'s `!Number.isInteger(...) || ... < 0` (non-integer and negative both tested), `options` present/absent, `thiefLevel > victimLevel` true/false, and `assertLevel(options.thiefLevel)` when `options` is given.
