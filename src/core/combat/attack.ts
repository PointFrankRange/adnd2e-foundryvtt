...
export interface AttackModifierInput {
  ...
  /**
   * 0 if proficient; the class non-proficiency penalty if not (half of it,
   * rounded up, for a "related" weapon when the Skills & Powers
   * expanded-proficiencies rule is on); tiered mastery bonus if
   * specialized/mastered. Tier 1 (Specialized) is the pre-existing,
